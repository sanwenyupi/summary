...
function getType(obj){
  ...
  // return Object.prototype.toString.call(obj).slice(7).replace(']','')
}
```
...
new Array(10).fill(0)
// 2.
Array.from(new Array(10), item => 0)
```
...
+ 父子通信：$parent和$children

+ $refs，获取DOM元素或者组件实例，DOM元素直接this.$refs[refsName]操作元素，组件实例需要this.$refs[refsName].$el操作元素，也可以执行子组件中的方法
...
![](https://image.fundebug.com/2019-06-26-01.png)
...
![](https://image.fundebug.com/2019-06-26-02.png)
...
v-if惰性渲染，后面的条件为false时，对应的元素以及其子元素都不会被渲染，没有标签出现在DOM中

v-show
跟v-if差不多，元素始终会渲染，为false时，给元素设置display:none
...
+ 高内聚、低耦合
+ 和业务有关的数据，不要在公用组件中处理、获取，数据从父组件传入
  + 为了和其他业务组件解耦，子组件尽量不产生数据，需要用到的数据由使用该组件的父组件来传入，并操作
+ 在父组件中处理事件
+ 插槽slot
+ 不要依赖vuex，耦合度太高
+ 注意兼容性
+ 可扩展性、易用性
+ 功能配置尽量留给开发者定义，可用于二次开发
+ 定义的公共方法可以使用defineProperty设置configurable, writable, enumerable来封装
...
<Detail>
    ...
    <!-- v-slot只能在template当中使用，简写为#footer，目前比较推荐这种写法，上面的那种在2.6.0被废弃 -->
...
```js
* loader负责扩展webpack可解析、打包的文件类型，.less -> css
* plugin负责打包优化、压缩、文件管理、环境注入等更丰富的功能，它针对的是loader结束后，webpack的整个打包过程，不直接操作文件，基于事件机制工作，监听打包过程的某些节点，执行广泛的任务。
```

### webpack打包ts

```
npm init -y（初始化package.json) -> webpack.config.js（新建文件配置webpack） -> tsc --init（生成tsconfig.json配置ts） -> npm build（在package.json中新增命令脚本"build": "webpack"）
```
...
```
1. 将新语法转换成旧语法
2. 将新对象，新技术，新类等旧浏览器不支持的语法转换为支持的
```

### tsconfig.json和babel的区别

```
tsconfig.json中的target是将ts代码转换为对应版本的js代码
babel则是将一些promise等新语法转换为浏览器兼容的js代码，是tsconfig中无法做到的
```

### webpack常用的plugin

1. html-webpack-plugin。创建一个html模板，自动引入js css。
2. clean-webpack-plugin。每次打包前删除dist文件夹。
3. webpack.optimize.CommonsChunkPlugin / optimization.SplitChunks。提取公共代码，优化加载速度。
4. IgnorePlugin。过滤打包文件，减少打包体积。
5. uglifyjs-webpack-plugin。用于压缩js文件。
6. optimize-css-assets-webpack-plugin。压缩css样式。
7. imagemin-webpack-plugin。压缩图片。
...
7. 通过this.$store.dispatch('action中方法')来执行异步操作 -> 
   action中通过参数({commit}, context){commit('mutation中方法'), context}来修改状态
```
...
2. const保证变量指向内存地址不得改动
3. const的与let的特点基本相同，同样不存在变量提升，存在暂时性死区，只能在声明后使用

let,const声明的变量不在window对象上
```
...
// 无论返回的携带有promise执行结果的数组中的顺序应该与传入时的顺序一致。解决方案其实就是，通过当前遍历传入参数的索引来作为results的索引，往里面添加。
...
绕开类型检查的方法：1.用变量 2.类型断言 3.字符串索引签名
...
+ 语法: const xxx = ref(initValue):
  ...
  - js中操作数据: xxx.value
  - 模板中操作数据: 不需要.value
+ 一般用来定义一个基本类型的响应式数据，如果定义引用类型，其实还是使用的reactive实现的响应式，即通过proxy实现的
+ 基于Object.definProperty和getter/setter实现响应式
```vue
<template>
  <h2>{{count}}</h2>
  <hr>
...
+ 问题
  ...
  + 直接通过下标替换元素或更新length, 界面不会自动更新 
    ...
    + ```js
      // 修改某个元素
      this.$set(this.arr, 0, '新值')
      
      this.arr.splice(0, 1, '新值')
      ```
##### Vue3的响应式：

+ 核心
  + 通过Proxy(代理):【 拦截】对data任意属性的任意(13种)操作, 包括属性值的读写, 属性的添加, 属性的删除等...
  + 通过 Reflect(反射): 动态对源对象的相应属性进行特定的【操作】
  + 文档:
...
- 本质上是一个函数，使用Vue3的组合API封装的可复用的功能函数
