...
- 浏览器是多进程的，没打开一个tab，都创建了一个进程（有些进程也会合并）
...
+ JS阻塞页面加载
+ Web Worker，JS多线程？（×）
  + 创建woker时，JS引擎向浏览器申请开一个子线程（该子线程是由浏览器开的，完全受JS主线程控制，且不能操作DOM）
...
+ $refs，获取DOM元素或者组件实例，DOM元素直接this.$refs[refsName]操作元素，组件实例需要this.$refs[refsName].$el操作元素，可以执行子组件中的方法
