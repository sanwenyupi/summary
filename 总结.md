https://juejin.cn/post/6948227795059212318#heading-1
...
### 1.1.2 JS优化
    ...
    添加dom元素时可以createElement，最后一起加入
    添加样式时增加class属性，减少重排
...
## 2.2 定位
    position:  
    static(默认),
    relative(位置不变，不脱离文档流), 
    absolute(脱离文档流，相对于开启定位的祖先元素，如果都没有，则相对于body), 
    sticky(当元素到达某个位置时固定，脱离后移动。跟随窗口), 
    fixed(始终固定，相对body定位。侧边栏、广告图), 
    inherit(继承父元素的postion)
...
2. position(元素未知宽度)
上面的margin改为：transform: translate(-50%, -50%)
...
3.__proto__是个对象，它与其构造函数的显示原型对象只向同一个对象
...
当一个实例对象使用某个方法或属性时，会沿着原型链查找，首先在自身上，其次查找它的__proto__指向的其构造函数的原型对象，最后查找Object.prototype
```
...
用JSON.stringify()将对象转成字符串，再用JSON.parse()把字符串解析成对象。
缺点：这种方法可以实现数组和对象和基本数据类型的深拷贝，但不能处理函数。
因为JSON.stringify()方法是将一个javascript值转换我一个JSON字符串，不能接受函数。其他影响如下：
...
2. 左边是对象，调用[].toString()转换为空字符串''，再转换为false，再转换为number 0
3. 左右两边 0==0，因此为true
```
...
4. 如果一个操作数是对象，另一个不是，则调用对象的toString方法，用得到的基本类型根据前几条规则比较
```
...
【数据描述符】：
1.configurable: 表示对象的属性是否可以被删除，以及除 value 和 writable 特性外的其他特性是否可以被修改，把 configurable 修改成false 是单向操作， 无法撤销！
2.enumerable：定义了对象的属性是否可以在 for...in 循环和 Object.keys() 中被枚举
【上面两种描述符是共享的】
3.value：该属性对应的值
4.writable：属性设置为 false 时，该属性被称为“不可写的”。它不能被重新赋值。
【存取描述符】：
1.getter
2.setter
...
3. 双层for循环+splice
NaN和{}不去重，null直接删掉
4. 创建新数组，for循环原数组，用indexOf判断新数组是否包含当前元素，没有就push，有了就跳过
5. Map
...
```
什么是同源策略？
浏览器为了保证用户信息安全，防止恶意网站窃取数据，禁止不同域之间的JS进行交互，对浏览器而言，只要域名、协议、端口其中一个不同就会引发同源策略，从而限制它们之间的交互：
...
```js
CORS定义了必须在访问跨域资源时，浏览器和服务器该如何沟通。
CORS背后的基本思想就是使用自定义的HTTP头部让浏览器与服务器进行沟通，从而决定请求或响应是应该成功还是失败。

普通跨域请求：服务器端设置Access-Control-Allow-Origin
...
```
父子：
props 和$emit 父组件向子组件传递数据是通过 prop 传递的，子组件传递数据给父组件是通过$emit 触发事件来做到的，子组件$emit传的参数，在父组件中使用子组件时自定义事件，@后不需要传参数，在methods中定义时接收参数。
...
## 5.2 事件循环相关输出题
    00知识总结 -> Promise+async await -> Promise输出题
```js
setTimeout(function () {
  console.log("1");
}, 0);
